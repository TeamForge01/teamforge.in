...
export async function generateUserProfile(answers: string[]) {
  ...
  console.log("Gemini Response (generateUserProfile):", response.text);
  try {
    ...
    throw new Error("AI synthesis returned invalid data format.");
  }
}

export async function validateIdea(idea: { title: string; description: string; problem: string; solution: string; targetAudience: string; revenueModel?: string; geography?: string }) {
...
export async function chatWithLearningAssistant(messages: { role: 'user' | 'assistant', content: string }[], userContext: any) {
